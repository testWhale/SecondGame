...
namespace nui
{
	void SceneView::resize(int32_t width, int32_t height) {
		mSize.x = width;
		mSize.y = height;
		//FrameBuffer is the windows buffer obj 
	...
	void SceneView::render()
	{
		...
		mFrameBuffer->unbind();
		// Set the window to dock to the center by default
		ImGui::SetNextWindowDockID(ImGui::GetID("Scene_View"), ImGuiCond_FirstUseEver);
